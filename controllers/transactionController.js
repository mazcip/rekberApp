...
const paymentCallback = async (req, res) => {
  try {
    ...
    // const calculatedSignature = crypto.createHash('md5').update(...).digest('hex');
    ...
    try {
      ...
      await sequelize.query(
        `UPDATE transactions 
         SET status = :status, payment_method = :payment_method, updated_at = NOW()
         WHERE id = :id`,
        {
          replacements: {
            status: newStatus,
      ...
      const transactionData = {
        ...
        product_name: (await sequelize.query(
          ...
          {
            replacements: { product_id: (await sequelize.query(
              'SELECT product_id FROM transactions WHERE id = :transaction_id',
...
const getUserTransactions = async (req, res) => {
  try {
    ...
    const transactions = await sequelize.query(
      `SELECT t.id, t.invoice_number, t.status, t.quantity, t.total_amount,
              t.created_at, t.completed_at, p.name as product_name, p.thumbnail_url,
              m.shop_name, u.username as merchant_username
       FROM transactions t
       JOIN products p ON t.product_id = p.id
       JOIN merchants m ON t.merchant_id = m.id
